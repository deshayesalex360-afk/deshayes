...
export async function POST(
  ...
  const body = (await req.json()) as { ratio?: string; clipId?: string };
  ...
  const rate = await enforceRateLimit(user.id, user.plan, "videos:export");
  if (!rate.allowed) {
  ...
  const quota = await enforceMonthlyQuota(user.id, user.plan, "exports");
  if (!quota.allowed) {
    return NextResponse.json(
  ...
  await incrementUsageCounter(user.id, "exports");
